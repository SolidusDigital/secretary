...
if "pytest" in sys.modules:
    pass
    # ENV = ENV_VALUES.TEST
elif os.getenv('BACKUP_SERVICE_MODE', None) == 'RESTORE':
    ENV = ENV_VALUES.RESTORE
...
if ENV == ENV_VALUES.PROD:
    ...
    MSSQL_USER = 'sa'
    BACKUP_PATH = "/mnt/backup_drive/sql_server_backup"
    _MSSQL_MASTER_METRICS = 'tcp:st-dw-prod'
    RESTORE_PATH = None
    MSSQL_PASSWORD = '1A horse cannot unlock sheeps!'
    MSSQL_TrustedConnection = 'no'
    MSSQL_st_file_DB = 'st_file'
    BACKUP_SERVICE_MODE = 'BACKUP'
    DB_NAMES_TO_RESTORE_OR_BACKUP = ['st_file', 'st_metrics']
    METRICS_DB = 'st_metrics'
    
elif ENV == ENV_VALUES.RESTORE:
    _MSSQL_iac_TEST_IP = 'tcp:sear-prod'
    _MSSQL_MASTER_METRICS = 'tcp:st-dw-prod'
    RESTORE_PATH = "/mnt/backup_drive/sql_server_backup"
    BACKUP_PATH = None
    MSSQL_USER = 'sa'
    MSSQL_PASSWORD = '1A horse cannot unlock sheeps!'
    MSSQL_TrustedConnection = 'no'
    MSSQL_st_file_DB = 'master'
    BACKUP_SERVICE_MODE = 'RESTORE'
    DB_NAMES_TO_RESTORE_OR_BACKUP = ['st_file', 'st_metrics']
    METRICS_DB = 'st_metrics'
...
def _build_odbc_p_str(hostname=_MSSQL_iac_TEST_IP):
    return _ODBC_P_STR_TEMPLATE.format(_determine_odbc_driver(), hostname, MSSQL_TrustedConnection)


MSSQL_P_STR = _build_odbc_p_str()
METRICS_MSSQL_P_STR = _build_odbc_p_str(_MSSQL_MASTER_METRICS)
print(MSSQL_P_STR)
print(METRICS_MSSQL_P_STR)
print('ENV:', ENV)
