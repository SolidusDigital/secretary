...
class Connection:
    ...
    def _create_conn(self):
        conn = pyodbc.connect(Definitions.MSSQL_P_STR,
    ...
    def instance(cls):
        if cls._instance is None:
            ...
            cls._instance.conn = pyodbc.connect(Definitions.MSSQL_P_STR,
                                                user=Definitions.MSSQL_USER,
                                                password=Definitions.MSSQL_PASSWORD,
                                                database=Definitions.MSSQL_st_file_DB,
                                                autocommit=True,
...
class SqlSelect:
    ...
    def get_scrape_queue(self, limit=1000, retailer_key=None):
        sql_sp = f"""exec {st_file_db_name}.dbo.Get_MainScrapperQueue"""
        sql_params = {
            '@row_limit': limit,
    ...
    def Process_ResponseRaw(self, limit=10000, retailer_key=None, query=None):
        sql_sp = f"""exec {st_file_db_name}.dbo.Process_ResponseRaw"""
        sql_params = {
            '@run_limit': limit,
            '@retailer_key': retailer_key,
            }
    ...
    def insert_TireAtlasLibrary(self, limit=100):
        sql_sp = f"""exec {st_file_db_name}.dbo.insert_TireAtlasLibrary"""
        sql_params = {
            '@run_limit': limit,
            }
    ...
    def Maintain_ResponseRaw(self, limit=1000, retailer_key=None, query=None):
        sql_sp = f"""exec {st_file_db_name}.dbo.Maintain_ResponseRaw"""
        sql_params = {
            '@run_limit': limit,
            }
